...
<!-- 
		[조건]
			1. 사용자가 반드시 모든 입력 양식에 올바른 값을 입력한 후 submit 버튼을 클릭햐여야 한다.
		[구현방법]
			1) 서버에서 구현
...
<head>
	...
	/*function submitHandler(){
		...
		return false;	// 사용자 이벤트 처리가 false를 리턴하면 나중에 동작하는 default 이벤트 처리는 동작하지 않음.
						// 따라서 alert이 실행되고 서버에 요청을 보내지 않음
	}
	*/
	function submitHandler(){
		var userName = document.querySelector('#name').value;	// id가 name인 태그에 작성된 value 값 불러오기
		var password = document.querySelector('#password');
		var confirmPassword = document.querySelector('#confirmPassword');
		
		if(userName.length >=2 && password.value.length >=4 && confirmPassword.value == password.value){
			alert('회원가입 처리를 수행합니다.');
		}	
		else{
			alert('올바른 회원 정보를 입력하세요');
			password.value='';			// 민감한 정보 초기화
			confirmPassword.value='';
			return false;	// 리턴이 false이기 때문에 아무것도 수행하지 않고 멈춤
		}
	}
...
<body>
	...
	<form action="step3" method="POST" id="registerForm">
		<fieldset>
			...
			<label for = "email">이메일 : </label>	 required 는 boolean 속성, 반드시 값을 입력해야한다.
			<input type="email" name = "email" id = "email" value = "${registerRequest.email}"required/><br/>
			
			<label for = "name">이름 : </label>
			<input type="text" name = "name" id = "name" value = "${registerRequest.name}"required/><br/>
